...
const Navbar = () => {
    ...
    iconTl.current = gsap.timeline({paused:true}).to(topLineRef.current,{
        ...
        y:3.3,
        duration:0.3,
        ease:"power2.inOut"}).to(bottomLineRef.current,{
            rotate:-45,
            y:-3.3,
            duration:0.3,
            ease:"power2.inOut"
    ...
    gap-y-10 md:w-1/2 md:left-1/2'>
        ...
        gap-8 md:flex-row'>
            <div className='font-light'>
                ...
                <p className='text-xl tracking-widest lowercase'>Paneerudh@gmail.com</p>
            </div>
            <div className='font-light'>
                <p className='tracking-wider text-white/50'>Socials</p>
                <div className='flex flex-col flex-wrap md:flex-row
                gap-x-2'>{socials.map((socials,index)=>(<a 
                    key={index}
                    href={socials.href} 
                    className='text-sm
                leading-loose tracking-widest uppercase hover:text-white transition duration-300'>
                    {"{ "}
                    {socials.name}
                    {" }"}
                    </a>))}
